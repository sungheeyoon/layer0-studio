...
Templates live in `src/templates/<category>/<leaf>/` (β model since #6). Each Template is **self-contained**: own visual tokens (`tokens.ts`), own library of section components (`library/*.tsx` with `.meta.fieldsSchema`), own `template.ts` (the seed), own `index.tsx` renderer. Components are NOT shared across Templates — every Template owns its copies. This is deliberate (isolation > DRY — see [ADR-0001](./docs/adr/0001-beta-model-template-isolation.md)). The registry is auto-generated (`src/templates/_generated.ts` via `pnpm generate:templates`, hooked into predev/prebuild) — adding a directory is enough to register. **templateKey = `<category>-<leaf>` (concat, lowercase)**. Category is derived from the parent dir name, **Capitalized** (e.g. `corporate` → `Corporate`) — the canonical catalog form generated into `templateCategories`; `categoryLabel()` lowercases before the i18n lookup and `syncTemplates` reconciles the DB `category` to this value on UPDATE (so a slug renamed in code propagates). Currently 11 Templates ship across 9 Categories: `academy-default`, `cafe-{cozy,default}`, `corporate-default`, `fitness-default`, `interior-default`, `legal-default`, `medical-{clinic,default}`, `outdoor-default`, `wedding-default`. **`outdoor-default` (능선) and `medical-clinic` (온유의원) are Multi Site Type templates; the other 9 are Single.** (The earlier minimal Multi example `corporate-multipage` and `cafe-modern` were removed; the Multi rendering infrastructure `renderMultiSite` / `[[...slug]]` routing is in active use.) See "Site Types (Single / Multi)" below and [ADR-0007](./docs/adr/0007-single-multi-site-type-structural-union.md).

The `ContentModel` type (in `src/domain/entities/template.entity.ts`, formerly `TemplateJson` — see [ADR-0013](./docs/adr/0013-content-model-rename.md)) is the core data model — it flows from DB → editor → renderer. **Two axes of copy/share (don't conflate — see `CONTEXT.md` Flagged ambiguities):** when a User instantiates a Site, the Template's `content` is deep-copied (`structuredClone` in `create-site-from-template.usecase.ts`) into the Site's own `content` (per-Site **data** copy, ~KB); the **renderer code** is never copied — it is shared and loaded at serve time by `templateKey` via `loadTemplate()`. So template edits do not propagate to existing Sites, and "a Site is a copy" means its data, not its code.
...
**Site Types (Single / Multi)** (see [ADR-0007](./docs/adr/0007-single-multi-site-type-structural-union.md), implemented): `ContentModel` is a **structural union discriminated on `mode`** (`SiteMode = 'single' | 'multi'`), fixed at creation (a Single never *evolves* into a Multi). Narrow with `isSingleContent` / `isMultiContent`; iterate every section regardless of mode with `allSections()` (all in `template.entity.ts`).
...
- **Multi** (`mode:'multi'`): routable `pages: Page[]` (each carries `nav:{visible,label}` + `slug`) plus `shared:{header,footer}`. Rendered by `renderMultiSite.tsx` (`header → page.sections → footer`); served at `/site/[domain]/[[...slug]]` (empty slug = first/home page; `visible:false` → 404). Editor manages pages (tabs/reorder/toggles/label), no create/delete.
...
**`array` field type** (Phase 1, merged): components can declare repeating-item fields in their `fieldsSchema` (e.g. menu items, FAQ entries). The editor renders add/remove/reorder UI and recursively validates each item against its `itemSchema`. Phase 2 (Collections — separate table + RLS for blogs/notices) is intentionally deferred — see `docs/plans/PLAN_crud_array_field.md` for trigger conditions before opening that work.
...
- `012_remove_section_order.sql` — strips deprecated `section.order` from the content JSONB (Phase 6d cleanup; columns were named `template_json`/`site_json`/`template_snapshot` at the time — see migration 021)
