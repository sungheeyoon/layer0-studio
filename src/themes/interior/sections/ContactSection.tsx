...
import { ArrowRightIcon, PhoneIcon, LetterIcon, MapPinIcon } from './icons';
...
export default function ContactSection({ section }: ThemeSectionProps) {
  ...
  return (
    <section id="contact" className="py-28 lg:py-36 relative overflow-hidden" style={{ background: 'linear-gradient(150deg, #161008 0%, #0C0A08 60%, #120E06 100%)' }}>
      {/* Decorative glow */}
      <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse 70% 60% at 50% 50%, rgba(201,169,110,0.07) 0%, transparent 70%)' }}></div>
      {/* Grid lines */}
            ...
            <form onSubmit={(e) => e.preventDefault()} className="space-y-4 mb-6">
              <div className="grid grid-cols-2 gap-3">
                <div className="text-left">
                  ...
                  <input type="text" placeholder="홍길동" className="w-full px-4 py-3 rounded-xl text-[14px] text-[var(--i-cream)] placeholder-[var(--i-muted)] outline-none focus:ring-1 focus:ring-[var(--i-gold)] transition-all" style={{ background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.08)' }} />
                </div>
                <div className="text-left">
                  <label className="block text-[12px] text-[var(--i-muted)] mb-1.5">연락처</label>
                  <input type="tel" placeholder="010-0000-0000" className="w-full px-4 py-3 rounded-xl text-[14px] text-[var(--i-cream)] placeholder-[var(--i-muted)] outline-none focus:ring-1 focus:ring-[var(--i-gold)] transition-all" style={{ background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.08)' }} />
                </div>
              </div>
              <div className="text-left">
                <label className="block text-[12px] text-[var(--i-muted)] mb-1.5">서비스 유형</label>
                <select className="w-full px-4 py-3 rounded-xl text-[14px] outline-none focus:ring-1 focus:ring-[var(--i-gold)] transition-all appearance-none text-[var(--i-muted)]" style={{ background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.08)' }}>
