...
/**
 * ADR-0016 schema-first: the schema is the single source of truth and the
 * Content type is *derived* from it — there is no hand-written interface to
 * drift out of step. `as const` is load-bearing (it preserves `required: true`
 * and any `options` literals so `ValuesOf` can read them).
 */
const menuBentoSchema = {
...
const MenuBento: SectionComponent = function MenuBento({ section }: TemplateSectionProps) {
  ...
  const content = section.fields as unknown as MenuBentoContent;

  const label = content.eyebrow || '메뉴';
  const title = content.title || '매일 정성껏\n내리는 한 잔';
  const description = content.description || '';

  const programs = (content.items ?? []).map((item, idx) => ({
    // The item's own permanent id — not the array index, which changes on
    // reorder and used to make React reuse the wrong card (ADR-0016 §4-4).
    id: item.id,
...
MenuBento.meta = {
  ...
  category: 'menu',
  label: 'Menu (Bento)',
  // Same object the Content type was derived from — one declaration, two uses.
  fieldsSchema: menuBentoSchema,
  previewImage: '/component-previews/cafe/menu-bento.webp',
};

export { menuBentoSchema };
export type { MenuBentoContent };
