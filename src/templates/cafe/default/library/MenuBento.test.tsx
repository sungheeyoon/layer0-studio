...
function blockWith(content: MenuBentoContent): Section {
  return {
    id: 'menu-001',
    type: 'menu',
    visible: true,
    fields: content as unknown as Section['fields'],
  };
}
...
describe('MenuBento — schema-first rendering (ADR-0016)', () => {
  it('renders values straight from the Content, with no getFieldValue unwrapping', () => {
    render(<MenuBento section={blockWith(base)} />);
  ...
  it('falls back when optional Values are absent', () => {
    ...
    render(<MenuBento section={blockWith(bare)} />);
  ...
  it('drops items whose required title is empty rather than rendering a blank card', () => {
    ...
    render(<MenuBento section={blockWith(withBlank)} />);
