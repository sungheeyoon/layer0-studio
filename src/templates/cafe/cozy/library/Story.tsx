...
const Story: SectionComponent = function Story({ section }: TemplateSectionProps) {
  ...
  const pillars = [
    { title: content.f1Title, desc: content.f1Desc, icon: <LeafIcon size={22} className="text-[var(--c-terra)] fill-current" /> },
    { title: content.f2Title, desc: content.f2Desc, icon: <FireIcon size={22} className="text-[var(--c-terra)] fill-current" /> },
    { title: content.f3Title, desc: content.f3Desc, icon: <HandHeartIcon size={22} className="text-[var(--c-terra)] fill-current" /> },
  ].filter(p => p.title);
