      ...
      <div className={styles.container}>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-[var(--l-navy)] tracking-tight leading-tight whitespace-pre-line mb-6">
              {title}
            </h2>
          ...
          <div className="space-y-6">
            {reasons.map((r, i) => (
              <div key={i} className="flex gap-5 bg-white rounded-xl p-6 border border-stone-200 shadow-sm transition-shadow hover:shadow-md">
                <div className="w-12 h-12 bg-[var(--l-navy)] rounded-xl flex items-center justify-center flex-shrink-0">
                  {r.icon}
                </div>
                <div>
                  <h3 className="font-bold text-[var(--l-navy)] mb-1">{r.title}</h3>
